...
sal_table <- table(sal_clean$section, sal_clean$unittype)

chisq.test(sal_table)
```
The chi-squared test showed that there is a very small p-value (< 2.2e-16), meaning that there is a statistically significant relationship between forest type and channel unit for the Costal Giant Salamander. 
...
var.test(cc_weight, og_weight)
```
The F test shows a small p-value (1.439e-07), therefore I reject the null hypothesis that the variances are equal, suggesting the variances are not equal. 
...
# I transformed the data using a log transform, since the histogram shows that they are right skewed. Using this, I got a much higher p-value of 0.0053 and can now run the t-test.

t.test(log(sal_clean$weight_g) ~ sal_clean$section, var.equal = TRUE)

```
This t-test shows that we can reject the null hypothesis, as there is a small p-value (2.2e-16). This suggests that there is a statistically significant difference between the two means. Looking at the means of each group, we can tell that there is a higher mean weight in clear cut forests compared to old growth forests. 
...
```
Using the Shapiro-Wilks test and looking at the histograms, I can tell that the data is right skewed so I used a log transform to normalize it. 
```{r}
cor.test(log(trout_clean$length_1_mm), log(trout_clean$weight_g))
```
The correlation shows that there is a very significant correlation between body length and body mass. We know this because there is a very small p-value (2.2e-16) and a positive correlation coefficient of 0.99. 
...
pie_crab |>
  ...
  labs(x = "Latitude", y = "Carapace Width") +
  theme_classic()
```
The above box plot shows a general trend of a positive relationship between the two variables. As the mean carapace width increases, the latitude increases as well.
...
pie_crab |> 
  ggscatter(x = 'water_temp_sd', y = 'size', 
            ...
            add = "reg.line")
```
The p-value of this regression model is very large, suggesting that we should accept the null hypothesis. This also suggests that there is very little statistically significant correlation between water temperature and carpace width. 
...
```{r}
correlation <- cor(pie_crab[, c("latitude", "air_temp_sd", "water_temp_sd")])
print(correlation)
...
```
The correlation test shows that latitude and air temperature have a strong positive correlation, latitude and water temperature have a weak positive correlation, and water temperature and air temperature have a moderate positive correlation. The multiple linear regression shows that there is a small p-value, suggesting that we should reject the null and that the predictor variables have a statistically significant impact on carapace width. However, looking at the individual variable, latitude is the only one with a small enough p-value to accept the null. 
